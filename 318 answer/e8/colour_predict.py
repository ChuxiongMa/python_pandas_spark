...
def main():
    ...
    knn_lab_model = make_pipeline(FunctionTransformer(rgbTolab), KNeighborsClassifier(n_neighbors=5))

    svc_rgb_model = SVC(kernel='linear', C=6.5)
    svc_lab_model = make_pipeline(FunctionTransformer(rgbTolab), SVC(kernel='linear', C=6.5))
