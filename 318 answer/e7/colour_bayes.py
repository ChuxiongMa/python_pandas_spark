...
def main():
    ...
    rgb = X.as_matrix(columns=X.columns[0:3])
    rgb = rgb/255

    colour_words = y.as_matrix(columns=X.columns[3:4])

    X_train, X_test, y_train, y_test = train_test_split(rgb, colour_words)
    ...
    rgb_y_predicted = model_rgb.predict(X_test)
    print(accuracy_score(y_test, rgb_y_predicted))
    ...
    def rgbTolab(data):
        ...
        lab = rgb2lab(data_3d)
        return  lab.reshape(lab.shape[0],lab.shape[2])
