...
def pivot_months_pandas(data):
    ...
    data1 = pd.DataFrame({'month': month})
    data.append(data1)
    del data['date']
    del data['latitude']
    del data['longitude']
    del data['elevation']
    added_data = pd.concat([data, data1], axis=1)
    grouped_data = added_data.groupby(['name','month'])
    sum_data = grouped_data.aggregate('sum').reset_index()
    monthly = sum_data.pivot(index='name', columns='month', values='precipitation')

    count_data = grouped_data.aggregate('count').reset_index()
    counts = count_data.pivot(index='name', columns='month', values='station')
